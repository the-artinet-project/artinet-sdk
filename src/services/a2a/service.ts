...
export class Service implements A2A.Service {
    ...
    set contexts(contexts: A2A.Contexts) {
        this._contexts.list().then((list: A2A.Context[]) => {
    ...
    async execute({ engine, context }: { engine: A2A.Engine; context: A2A.Context }): Promise<void> {
    ...
    async getTask(params: A2A.TaskQueryParams, options?: A2A.ServiceOptions): Promise<A2A.Task> {
        const taskParams: A2A.TaskQueryParams = await validateSchema(A2A.TaskQueryParamsSchema, params);

        logger.info(`Service[getTask]:`, { taskId: taskParams.id });
        const task: A2A.Task | undefined = options?.task ?? (await this.tasks.get(taskParams.id));
    ...
    async cancelTask(params: A2A.TaskIdParams, options?: A2A.ServiceOptions): Promise<A2A.Task> {
        const taskParams: A2A.TaskIdParams = await validateSchema(A2A.TaskIdParamsSchema, params);
    ...
    async sendMessage(
        paramsOrMessage: A2A.MessageSendParams | A2A.MessageSendParams['message'] | string,
        options?: A2A.ServiceOptions,
    ): Promise<A2A.SendMessageSuccessResult> {
    ...
    protected async _sendMessage(
        ...
        const messageParams: A2A.MessageSendParams = await validateSchema(A2A.MessageSendParamsSchema, params);
    ...
    async *resubscribe(params: A2A.TaskIdParams, options?: A2A.ServiceOptions): AsyncGenerator<A2A.Update> {
        const taskParams: A2A.TaskIdParams = await validateSchema(A2A.TaskIdParamsSchema, params);
