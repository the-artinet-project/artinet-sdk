 ...
 */
export const isMessengerParams = (params: unknown): params is MessengerParams => {
 ...
 */
class Messenger implements Omit<Transport, 'getAuthenticatedExtendedAgentCard' | 'getExtendedAgentCard'> {
     ...
     */
    async sendMessage(
        params: A2A.MessageSendParams | describe.MessageSendParamsParams | describe.MessageParams,
        options?: RequestOptions,
    ): Promise<A2A.SendMessageSuccessResult> {
        const client = await this._client;
     ...
     */
    async getTask(params: A2A.TaskQueryParams, options?: RequestOptions): Promise<A2A.Task> {
        const client = await this._client;
     ...
     */
    async cancelTask(params: A2A.TaskIdParams, options?: RequestOptions): Promise<A2A.Task> {
        const client = await this._client;
     ...
     */
    async *resubscribeTask(params: A2A.TaskQueryParams, options?: RequestOptions): AsyncGenerator<A2A.Update> {
        try {
            const client = await this._client;
     ...
     */
    async supports(
        capability: 'streaming' | 'pushNotifications' | 'stateTransitionHistory' | 'extensions',
    ): Promise<boolean> {
    ...
    static async create({ baseUrl, headers, fallbackPath, factory, config }: MessengerParams): Promise<Messenger> {
        const _factory = {
