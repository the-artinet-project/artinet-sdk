...
export class Files extends Tasks {
    ...
    override async get(taskId: string): Promise<A2A.Task | undefined> {
        const task =
            (await super.get(taskId)) ?? (await this.readJsonFile(this.getTaskFilePath(taskId)).catch(() => undefined));
    ...
    override async list(): Promise<A2A.Task[]> {
        ...
        return Promise.all(taskIds.map((taskId) => this.get(taskId).catch(() => undefined))).then((tasks) =>
            tasks.filter((task) => task !== undefined),
        );
    }
}
