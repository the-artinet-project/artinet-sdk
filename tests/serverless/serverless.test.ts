...
function createMockEvent(method: string, path: string, body?: object): APIGatewayProxyEvent {
    return {
        httpMethod: method,
        path,
...
(isBun ? describe.skip : describe)('createServerlessHandler', () => {
    const agent = cr8({
    ...
    const handler: Handler = serve({ agent, basePath: '/a2a' }, { provider: 'aws' });
