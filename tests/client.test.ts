 ...
 */
import { test, describe, expect, beforeEach, beforeAll, afterAll } from '@jest/globals';

import { A2AClient, A2A, createMessenger } from '../src/index.js';
...
describe('A2AClient', () => {
    ...
    test('should handle network errors during HTTP request', async () => {
        ...
        const promise = async () => {
            await messenger.getTask({ id: 'test-task-123' });
        };
    ...
    test('should handle error during capability check', async () => {
        server.use(
            http.get('https://invalid-url.example.com/.well-known/agent-card.json', () => {
                return new HttpResponse(null, { status: 404 });
            }),
        ...
        ).rejects.toThrow('Failed to fetch Agent Card from https://invalid-url.example.com');
    });
    ...
    test('should handle JSON-RPC errors', async () => {
        ...
        await expect(messenger.getTask({ id: 'nonexistent-task' })).rejects.toThrow();
    });
    ...
    test('should check if a capability is supported', async () => {
        ...
        const hasStateTransitionHistory = await messenger.supports('stateTransitionHistory');
        expect(hasStateTransitionHistory).toBe(false);

        // Test the default case in the switch statement for uncovered branch
        const hasUnsupportedCapability = await messenger.supports('unknownCapability' as any);
