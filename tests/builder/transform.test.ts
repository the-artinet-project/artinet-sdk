...
describe("Transform Module Tests", () => {
  ...
  describe("Parts Transform", () => {
    ...
    it("should transform file parts", async () => {
      ...
      for await (const result of partsTransform(
      ...
      expect(results[0].status.message.parts[0].file.uri).toBe(
  ...
  describe("Message Transform", () => {
    ...
    it("should throw error for array of messages", async () => {
      ...
      await expect(async () => {
        for await (const _ of messageTransform(
          ["msg1", "msg2"] as any,
          context
        )) {
          // consume
        }
      }).rejects.toThrow("Array of messages is not supported");
    });
  });
  ...
  describe("Status Transform", () => {
    ...
    it("should transform status objects", async () => {
      ...
      for await (const result of statusTransform(
  ...
  describe("Task Transform", () => {
    ...
    it("should throw error for array of tasks", async () => {
      ...
      await expect(async () => {
        for await (const _ of taskTransform(
          ["task1", "task2"] as any,
          context
        )) {
          // consume
        }
      }).rejects.toThrow("Array of tasks is not supported");
    });
  });
  ...
  describe("Reply Handler", () => {
    ...
    it("should handle array return", async () => {
      ...
      for await (const result of transform.Reply(
        ["hello", "world"],
        context,
        partsTransform
      )) {
    ...
    it("should handle reply with carry return", async () => {
      ...
      const gen = transform.Reply(
        { reply: "hello", args: { carried: "value" } },
        context,
        partsTransform
      );
